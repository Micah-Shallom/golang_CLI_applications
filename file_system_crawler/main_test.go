...
func TestRun(t *testing.T) {
	testCases := []struct {
		...
		{name: "NoFilter", root: "testdata", cfg: config{exts: []string{""}, size: 0, list: true}, expected: "testdata\\dir.log\ntestdata\\dir2\\script.sh\n"},
		{name: "FilterExtensionMatch", root: "testdata", cfg: config{exts: []string{".log"}, size: 0, list: true}, expected: "testdata\\dir.log\n"},
		{name: "FilterExtensionSizeMatch", root: "testdata", cfg: config{exts: []string{".log"}, size: 10, list: true}, expected: "testdata\\dir.log\n"},
		{name: "FilterExtensionSizeNoMatch", root: "testdata", cfg: config{exts: []string{".log"}, size: 20, list: true}, expected: ""},
		{name: "FilterExtensionNoMatch", root: "testdata", cfg: config{exts: []string{".gz"}, size: 0, list: true}, expected: ""},
	}
...
func TestRunArchive(t *testing.T) {
	testCases := []struct {
		...
		{name: "ArchiveExtensionNoMatch", cfg: config{exts: []string{".log"}}, extNoArchive: ".gz", nArchive: 0, nNoArchive: 10},
		{name: "ArchiveExtensionMatch", cfg: config{exts: []string{".log"}}, extNoArchive: "", nArchive: 10, nNoArchive: 0},
		{name: "ArchiveExtensionNoMixed", cfg: config{exts: []string{".log"}}, extNoArchive: ".gz", nArchive: 5, nNoArchive: 5},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			pattern := filepath.Join(tempDir, filepath.Join(tempDir, fmt.Sprintf("*%s", tc.cfg.exts[0])))
